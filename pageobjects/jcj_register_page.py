...
from framework.base_base import BasePage
...
class RegisterPage(BasePage):
    ...
    def __init__(self, driver):
        ...
        def to_register(self):
            element = self.find_the_element(self.map_register['link_register'])
        ...
        def get_password_warn(self):
            reutrn self.find_the_element(self.map_register['input_reg_password']).get_attribute('value')
        ...
        def input_verify_code(self,code):
            self.typeIn(self.map_register['input_reg_sms'])
            time.sleep(1)
